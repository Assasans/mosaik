...
int main(int argc, char **argv) {
  ...
  // if((ret = decoder.open_input("file:///home/assasans/Downloads/leningrad_-_v-pitere-pit.mp3")) < 0) {
  ...
  if((ret = decoder.init_filters("atempo=1.5")) < 0) {
  ...
  while(true) {
    ret = decoder.read_frame([](float* chunk, int length, void* user) {
  ...
  while(true) {
    ...
    ret = decoder.flush_frame([](float* chunk, int length, void* user) {
      fprintf(stderr, "flush %d\n", length);
      print_frame(chunk, length);
    }, nullptr);
    fprintf(stderr, "flush ret: %s\n", av_err2str(ret));
    if(ret < 0 && ret != AVERROR(EAGAIN)) break;
  }
